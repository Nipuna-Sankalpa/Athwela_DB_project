    ...
    public function registerAction() {
        ...
        if ($process) {
            ...
            if ($confirmationEnabled) {
                ...
                $authUser = true;
                if ($role[0] == 'ROLE_ORG') {
                    $route = 'organization_profile_settings';
                    $query = "INSERT INTO organization (ID,email) VALUES ('$ID','$email')";
                    CustomQuery::getInstance()->customQuery($query);
                } elseif ($role[0] == 'ROLE_VOL') {
                    $route = 'volunteer_profile_settings';
                    $query = "INSERT INTO volunteer (ID,email) VALUES ('$ID','$email')";
                    CustomQuery::getInstance()->customQuery($query);
                }
            }

            $this->setFlash('fos_user_success', 'registration.flash.user_created');
            $url = $this->container->get('router')->generate($route, array('id' => $ID));
