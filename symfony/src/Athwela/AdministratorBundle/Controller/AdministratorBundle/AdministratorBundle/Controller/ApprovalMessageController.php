...
//                    'Msg' => $Msg,
    ...
    public function sendOrgMsgAction(Request $request) {
        ...
        if ($request->getMethod('POST')) {
            $organization_ID = $request->get('emailOrg');
            $query = "SELECT ID FROM organization  WHERE email='" . $organization_ID . "'";
            $result = CustomQuery::getInstance()->customQuery($query);
            ...
            $admin_ID = $user->getEmail();

            $query = "SELECT ID FROM admin  WHERE email='" . $admin_ID . "'";
    ...
    public function sendVolMsgAction(Request $request) {
        $user = $this->container->get('security.context')->getToken()->getUser();
        if ($request->getMethod('POST')) {
            $volunteer_ID = $request->get('emailVol');
            $query = "SELECT ID FROM volunteer  WHERE email='" . $volunteer_ID . "'";
            $result = CustomQuery::getInstance()->customQuery($query);
