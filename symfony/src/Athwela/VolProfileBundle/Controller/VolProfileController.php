...
//    along with the route.
    ...
    public function getAdmin($conn, $entity) {
        $admin = CustomQuery::getInstance()->customQuery($conn, 'SELECT a.first_name, a.last_name, a.email FROM `admin` as a, volunteer as v where a.ID = v.a_ID and v.ID = ' . $entity->getId());
        $temp = new Admin();
