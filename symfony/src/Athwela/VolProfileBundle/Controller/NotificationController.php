...
class NotificationController extends ContainerAware {
    ...
    public function getMessages($entity) {
        $i = 0;
        $messages[][] = null;
        $message = CustomQuery::getInstance()->customQuery('SELECT * FROM vol_admin_msg where msgStatus = "notRead"  and ID = ' . $entity->getId());
        
        while ($row = mysqli_fetch_row($message)) {
            for ($index = 0; $index < count($row); $index++) {
                $messages[$i][$index] = $row[$index];
            }
            CustomQuery::getInstance()->customQuery('Update vol_admin_msg set msgStatus = "Read" where ID = ' . $entity->getId().' and date = "' . $row[2].'"');
            $i++;
        }
        return $messages;
    }
