...
class OrgNotificationController extends ContainerAware {
    ...
    public function getMessages($entity) {
        $i = 0;
        $messages[][] = null;
        $message = CustomQuery::getInstance()->customQuery('SELECT * FROM admin_org_messages where msgStatus = "notRead" and o_ID = ' . $entity->getId());
        $admin = null;
        while ($row = mysqli_fetch_row($message)) {
            ...
            CustomQuery::getInstance()->customQuery('Update admin_org_messages set msgStatus = "Read" where o_ID = ' . $entity->getId() . ' and sent_time = "' . $row[2] . '"');
            $i++;
        }
        return $messages;
    }
    ...
    public function messagesendAction(Request $request, $email) {
        if ($request->getMethod() == "POST") {
            ...
            return $this->container->get('templating')->renderResponse('OrgProfileBundle:OrgProfile:message.html.twig', array('entity' => $entity, 'sent' => $sent, 'admins' => $admins));
        }
    }
}
